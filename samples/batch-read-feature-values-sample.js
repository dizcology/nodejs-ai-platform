...
async function main(featurestore) {
  ...
  async function batchReadFeatureValuesSample() {
    const request = {
      featurestore,
    };

    const [response] = await client.batchReadFeatureValues(request);
  ...
  await batchReadFeatureValuesSample();
