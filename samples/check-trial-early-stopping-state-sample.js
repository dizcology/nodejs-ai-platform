...
async function main(trialName) {
  ...
  async function checkTrialEarlyStoppingStateSample() {
    const request = {
      trialName,
    };
    const request = {
    ...
    const [response] = await client.checkTrialEarlyStoppingState(request);
  ...
  await checkTrialEarlyStoppingStateSample();
