...
async function main(project, location, metadataStore) {
  ...
  async function deleteMetadataStoreSample() {
    const name = `projects/${project}/locations/${location}/metadataStores/${metadata_store}`;

    const request = {
      name,
    };

    const [response] = await client.deleteMetadataStore(request);
  ...
  await deleteMetadataStoreSample();
